...
client.on('ready', () => {
  ...
  client.user.setActivity('Слава Кабанов', {type: 'WATCHING'})
});
...
client.on('message', async msg => {
 ...
 if (['eval', 'евал'].includes(cmd) && ['321268938728144906', '341988428457705482'].includes(msg.author.id)) {
  ...
  if(['status', 'статус'].includes(command) && ['321268938728144906', '341988428457705482']") {
        let new_args = args;
        ...
        if (['играет в', 'играет', 'play', 'playing', '0'].includes(type.toLowerCase()))
            ...
            real_type = 3;
        else return message.channel.send(`Ебобо? -> \`${type.replace(/` /g, "\'")}\` не существует`);
